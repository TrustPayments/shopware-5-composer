...
class Account implements SubscriberInterface
{
    ...
    public function onOrdersAction(\Enlight_Event_EventArgs $args)
    {
        ...
        foreach ($orderData as $orderKey => $order) {
            if (isset($order['id'])) {
                ...
                if ($transactionInfo instanceof TransactionInfo) {
                    $orderData[$orderKey]['trustPaymentsTransaction'] = $this->getTransactionVariables($transactionInfo);
                }
            }
        }
        ...
        $view->extendsTemplate('frontend/account/trust_payments_payment/order.tpl');
    }
    ...
    private function isDownloadInvoiceAllowed(TransactionInfo $transactionInfo)
    {
        ...
        $pluginConfig = $this->configReader->getByPluginName('TrustPaymentsPayment', $transactionInfo->getOrder()->getShop());
    ...
    private function getTransactionInfoByOrder($orderId)
    {
        ...
        if ($mapping instanceof OrderTransactionMapping) {
            return $this->transactionInfoService->updateTransactionInfoByOrder($this->container->get('trustpayments_payment.transaction')
