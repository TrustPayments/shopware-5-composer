...
class Transaction extends AbstractOrderRelatedSubscriber
{
    ...
    {
        return [
            'TrustPayments_Payment_Webhook_Transaction' => 'handle'
        ];
    }
    ...
    private function authorize(Order $order, \TrustPayments\Sdk\Model\Transaction $transaction)
    {
    ...
    private function complete(Order $order, \TrustPayments\Sdk\Model\Transaction $transaction)
    {
    ...
    private function decline(Order $order, \TrustPayments\Sdk\Model\Transaction $transaction)
    {
    ...
    private function failed(Order $order, \TrustPayments\Sdk\Model\Transaction $transaction)
    {
        try {
    ...
    private function fulfill(Order $order, \TrustPayments\Sdk\Model\Transaction $transaction)
    {
        if ($order->getPaymentStatus()->getId() == Status::PAYMENT_STATE_RESERVED) {
            $order->setPaymentStatus($this->getStatus(Status::PAYMENT_STATE_COMPLETELY_INVOICED));
        }
    ...
    private function voided(Order $order, \TrustPayments\Sdk\Model\Transaction $transaction)
    {
