...
class Backend implements SubscriberInterface
{
    ...
    {
        $args->getSubject()->View()->extendsTemplate('backend/trust_payments_payment_index/index.tpl');

        $args->getSubject()
            ->get('template')
            ->addTemplateDir($this->container->getParameter('trust_payments_payment.plugin_dir') . '/Resources/views/');

        if ($args->getRequest()->getActionName() === 'index') {
            $args->getSubject()
                ->View()
                ->extendsTemplate('backend/trust_payments_payment_manual_tasks/app.js');
        }
    }
    ...
    public function onPreDispatchBackendOrder(\Enlight_Event_EventArgs $args)
    {
        ...
        $args->getSubject()
            ->get('snippets')
            ->addConfigDir($this->container->getParameter('trust_payments_payment.plugin_dir') . '/Resources/snippets/');
    }
    ...
    public function onPostDispatchBackendOrder(\Enlight_Event_EventArgs $args)
    {
        switch ($args->getRequest()->getActionName()) {
            case 'index':
                $args->getSubject()
                    ->View()
                    ->extendsTemplate('backend/trust_payments_payment_order/app.js');
                break;
            case 'load':
                $args->getSubject()
                    ->View()
                    ->extendsTemplate('backend/trust_payments_payment_order/view/window.js');
                $args->getSubject()
                    ->View()
                    ->extendsTemplate('backend/trust_payments_payment_order/model/order.js');
                break;
            case 'getList':
                /* @var Plugin $plugin */
                ...
                foreach ($data as $key => $order) {
                    ...
                    if ($payment instanceof Payment) {
                        $data[$key]['trustpayments_payment'] = ($payment->getPluginId() == $plugin->getId());
                    }
                }
                $args->getSubject()
                    ->View()
