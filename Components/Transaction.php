...
class Transaction extends AbstractService
{
    ...
    public function __construct(ContainerInterface $container, ModelManager $modelManager, ConfigReader $configReader, ApiClient $apiClient, LineItem $lineItem, PaymentMethodConfigurationService $paymentMethodConfigurationService, TransactionInfoService $transactionInfoService, Session $sessionService)
    {
        ...
        $this->transactionService = new \TrustPayments\Sdk\Service\TransactionService($this->apiClient);
        $this->transactionIframeService = new \TrustPayments\Sdk\Service\TransactionIframeService($this->apiClient);
        $this->transactionPaymentPageService = new \TrustPayments\Sdk\Service\TransactionPaymentPageService($this->apiClient);
    }
    ...
    public function handleTransactionState($spaceId, $transactionId)
    {
        ...
        $this->container->get('trustpayments_payment.subscriber.webhook.transaction')->process($transaction);
    }
    ...
    {
        $baseUrl = $this->container->getParameter('trust_payments_payment.base_gateway_url');

        $pluginConfig = $this->configReader->getByPluginName('TrustPaymentsPayment', $this->container->get('shop'));
        $spaceId = $pluginConfig['spaceId'];

        $deviceId = Shopware()->Front()
            ->Request()
            ->getCookie('trustpayments_device_id');
        if (empty($deviceId)) {
            $deviceId = $this->generateDeviceId();
            Shopware()->Front()
                ->Response()
                ->setCookie('trustpayments_device_id', $deviceId);
        }
    ...
    public function getPossiblePaymentMethodsByBasket()
    {
        ...
        if (static::$possiblePaymentMethodByBasketCache == null) {
            if (! $this->isBasketTransactionChanged() && isset($session['trustpayments_payment.possible_payment_methods']) && ! empty($session['trustpayments_payment.possible_payment_methods'])) {
                $paymentMethods = $session['trustpayments_payment.possible_payment_methods'];
            } else {
                $transaction = $this->getTransactionByBasket();
                try {
                    $paymentMethods = $this->callApi($this->apiClient, function () use ($transaction) {
                ...
                $session['trustpayments_payment.possible_payment_methods'] = $paymentMethods;
            }
    ...
    public function createTransaction(Order $order, $confirm = false)
    {
        ...
        if ($existingTransaction instanceof \TrustPayments\Sdk\Model\Transaction) {
            return $this->updateTransaction($order, $existingTransaction->getId(), $existingTransaction->getLinkedSpaceId(), $confirm);
        } else {
            $transaction = new \TrustPayments\Sdk\Model\TransactionCreate();
            $transaction->setCustomersPresence(\TrustPayments\Sdk\Model\CustomersPresence::VIRTUAL_PRESENT);
    ...
    private function getTransactionHash()
    {
        /* @var \Enlight_Components_Session_Namespace $session */
        $session = $this->container->get('session');
        if (isset($session['trustpayments_payment.basket_transaction_data']) && ! empty($session['trustpayments_payment.basket_transaction_data'])) {
            return $session['trustpayments_payment.basket_transaction_data'];
        } else {
            return null;
        }
    }

    private function updateTransactionHash($spaceId, $transactionId, \TrustPayments\Sdk\Model\AbstractTransactionPending $transaction)
    {
        /* @var \Enlight_Components_Session_Namespace $session */
        $session = $this->container->get('session');
        $session['trustpayments_payment.basket_transaction_data'] = $this->generateTransactionHash($spaceId, $transactionId, $transaction);
    }
    ...
    private function isBasketTransactionChanged()
    {
        $orderTransactionMapping = $this->getBasketTransactionMapping();
        if ($orderTransactionMapping instanceof OrderTransactionMapping) {
            $transaction = new \TrustPayments\Sdk\Model\TransactionPending();
    ...
    private function isTransactionChanged($spaceId, $transactionId, \TrustPayments\Sdk\Model\AbstractTransactionPending $transaction)
    {
    ...
    private function generateTransactionHash($spaceId, $transactionId, \TrustPayments\Sdk\Model\AbstractTransactionPending $transaction)
    {
        $serializer = new \TrustPayments\Sdk\ObjectSerializer();
    ...
    private function addDeviceSessionIdentifier(\TrustPayments\Sdk\Model\AbstractTransactionPending $transaction)
    {
     ...
     * @param Order $order
     */
    private function assembleTransactionData(\TrustPayments\Sdk\Model\AbstractTransactionPending $transaction, Order $order)
    {
        ...
        if (! ($transaction instanceof \TrustPayments\Sdk\Model\TransactionCreate)) {
            $transaction->setSuccessUrl($this->getUrl('TrustPaymentsPaymentTransaction', 'success', null, null, [
                'spaceId' => $pluginConfig['spaceId'],
                'transactionId' => $transaction->getId()
            ]) . '?utm_nooverride=1');
            $transaction->setFailedUrl($this->getUrl('TrustPaymentsPaymentTransaction', 'failure', null, null, [
                'spaceId' => $pluginConfig['spaceId'],
                'transactionId' => $transaction->getId()
            ]) . '?utm_nooverride=1');
        }
     ...
     * @param \TrustPayments\Sdk\Model\AbstractTransactionPending $transaction
     */
    private function assembleBasketTransactionData(\TrustPayments\Sdk\Model\AbstractTransactionPending $transaction)
    {
        /* @var Shop $shop */
    ...
    private function updateOrCreateTransactionMapping(\TrustPayments\Sdk\Model\Transaction $transaction, Order $order)
    {
    ...
    private function updateOrCreateBasketTransactionMapping(\TrustPayments\Sdk\Model\Transaction $transaction)
    {
        if ($this->modelManager->getConnection()->isTransactionActive()) {
            throw new \Exception('This method cannot be called from within a database transaction.');
        }
