...
use TrustPaymentsPayment\Components\ArrayBuilder\PaymentMethod as PaymentMethodArrayBuilder;
use TrustPaymentsPayment\Components\ArrayBuilder\LineItemVersion as LineItemVersionArrayBuilder;
...
use TrustPaymentsPayment\Components\ArrayBuilder\Refund as RefundArrayBuilder;
...
class TransactionInfo extends AbstractArrayBuilder
{
    ...
    public function build()
    {
        $result = [
            ...
            'currencyDecimals' => $this->container->get('trustpayments_payment.provider.currency')->getFractionDigits($this->transactionInfo->getCurrency()),
    ...
    {
        return $this->container->getParameter('trust_payments_payment.base_gateway_url') . '/s/' . $this->transactionInfo->getSpaceId() . '/payment/transaction/view/' . $this->transactionInfo->getTransactionId();
    }
    ...
    {
        return $this->container->getParameter('trust_payments_payment.base_gateway_url') . '/s/' . $this->transactionInfo->getSpaceId() . '/payment/customer/transaction/view/' . $this->transactionInfo->getTransactionId();
    }
    ...
    {
        return $this->container->get('trustpayments_payment.resource')->getResourceUrl($this->transactionInfo->getImage(), $this->transactionInfo->getLanguage(), $this->transactionInfo->getSpaceId(), $this->transactionInfo->getSpaceViewId());
    }
    ...
    private function canReviewTransaction()
    {
        try {
            ...
            $deliveryIndication = $this->container->get('trustpayments_payment.delivery_indication')->getDeliveryIndication($this->transactionInfo);
            if ($deliveryIndication != null) {
                return $deliveryIndication->getState() == \TrustPayments\Sdk\Model\DeliveryIndicationState::MANUAL_CHECK_REQUIRED;
            } else {
                return false;
            }
        } catch (\Exception $e) {
    ...
    {
        if ($this->transaction != null && $this->transaction->getState() == \TrustPayments\Sdk\Model\TransactionState::AUTHORIZED) {
            return true;
        } else {
            return false;
        }
    ...
    private function canRefund()
    {
        ...
        foreach ($this->refunds as $refund) {
            if (in_array($refund->getState(), [
                \TrustPayments\Sdk\Model\RefundState::MANUAL_CHECK,
                \TrustPayments\Sdk\Model\RefundState::PENDING
            ])) {
                return false;
            }
        }

        if ($this->container->get('trustpayments_payment.line_item')->getTotalAmountIncludingTax($this->container->get('trustpayments_payment.refund')->getRefundBaseLineItems($this->invoice, $this->refunds)) <= 0) {
            return false;
        }
