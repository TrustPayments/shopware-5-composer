...
use TrustPaymentsPayment\Components\Provider\Currency as CurrencyProvider;
...
class LineItem extends AbstractService
{
    ...
    public function collectLineItems(Order $order)
    {
        ...
        foreach ($details as $detail) {
            ...
            $lineItem->setShippingRequired($type == \TrustPayments\Sdk\Model\LineItemType::PRODUCT && ! $detail->getEsdArticle());
            ...
            if ($order->getDispatch() instanceof \Shopware\Models\Dispatch\Dispatch) {
                ...
                $shippingMethodName = $this->container->get('snippets')
                    ->getNamespace('frontend/trustpayments_payment/main')
    ...
    public function collectBasketLineItems()
    {
        ...
        foreach ($basketData['content'] as $basketRow) {
            ...
            $lineItem->setShippingRequired($type == \TrustPayments\Sdk\Model\LineItemType::PRODUCT && ! $basketRow['esdarticle']);
        ...
        $pluginConfig = $this->configReader->getByPluginName('TrustPaymentsPayment', Shopware()->Shop());
    ...
    private function getType($mode, $price)
    {
        switch ($mode) {
            ...
            case static::ORDER_DETAIL_MODE_BUNDLE_DISCOUNT:
                return \TrustPayments\Sdk\Model\LineItemType::DISCOUNT;
            case static::ORDER_DETAIL_MODE_PAYMENT_SURCHARGE_DISCOUNT:
                if ($price > 0) {
                    return \TrustPayments\Sdk\Model\LineItemType::FEE;
                } else {
                    return \TrustPayments\Sdk\Model\LineItemType::DISCOUNT;
                }
            ...
            default:
                return \TrustPayments\Sdk\Model\LineItemType::PRODUCT;
        }
    }
     ...
     * @return \TrustPayments\Sdk\Model\LineItemCreate
     */
    private function cleanLineItem(\TrustPayments\Sdk\Model\LineItemCreate $lineItem)
    {
    ...
    private function adjustLineItems(array &$lineItems, $expectedSum, $currency)
    {
        ...
        $adjustmentLineItem->setType(
            $difference > 0 ? \TrustPayments\Sdk\Model\LineItemType::FEE : \TrustPayments\Sdk\Model\LineItemType::DISCOUNT);
