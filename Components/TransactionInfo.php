...
use TrustPayments\Sdk\Model\PaymentMethod;
use TrustPayments\Sdk\Model\Refund as RefundModel;
use TrustPayments\Sdk\Model\TransactionInvoice;
use TrustPayments\Sdk\Model\TransactionLineItemVersion;
use TrustPaymentsPayment\Components\ArrayBuilder\TransactionInfo as TransactionInfoArrayBuilder;
...
class TransactionInfo extends AbstractService
{
     ...
     */
    public function updateTransactionInfoByOrder(\TrustPayments\Sdk\Model\Transaction $transaction, Order $order)
    {
     ...
     * @return TransactionInfoModel
     */
    public function updateTransactionInfo(\TrustPayments\Sdk\Model\Transaction $transaction, $orderId, $shopId, $paymentId, Customer $customer)
    {
        try {
            ...
            if ($transaction->getState() == \TrustPayments\Sdk\Model\TransactionState::FAILED || $transaction->getState() == \TrustPayments\Sdk\Model\TransactionState::DECLINE) {
    ...
    private function loadTransactionInfo(\TrustPayments\Sdk\Model\Transaction $transaction, $orderId)
    {
    ...
    private function updateCustomerData(Customer $customer, \TrustPayments\Sdk\Model\Transaction $transaction)
    {
    ...
    private function getChargeAttempt(\TrustPayments\Sdk\Model\Transaction $transaction)
    {
        return $this->callApi($this->apiClient, function () use ($transaction) {
            $chargeAttemptService = new \TrustPayments\Sdk\Service\ChargeAttemptService($this->apiClient);
     ...
     * @return string
     */
    private function getPaymentMethodImage(\TrustPayments\Sdk\Model\Transaction $transaction, $paymentId)
    {
        ...
        /* @var \TrustPaymentsPayment\Components\Provider\PaymentConnector $connectorProvider */
        $connectorProvider = $this->container->get('trustpayments_payment.provider.payment_connector');
        $connector = $connectorProvider->find($transaction->getPaymentConnectorConfiguration()
            ->getConnector());

        /* @var \TrustPaymentsPayment\Components\Provider\PaymentMethod $methodProvider */
        $methodProvider = $this->container->get('trustpayments_payment.provider.payment_method');
    ...
    {
        try {
            $paymentMethod = $this->container->get('trustpayments_payment.provider.payment_method')->find($transactionInfo->getPaymentMethodId());
            if ($paymentMethod instanceof PaymentMethod) {
                return $paymentMethod;
            }
        } catch (\Exception $e) {
        }
        // If payment methods cannot be loaded from Trust Payments, information about the payment method cannot be displayed.
        return null;
    }
    ...
    {
        try {
            $transaction = $this->container->get('trustpayments_payment.transaction')->getTransaction($transactionInfo->getSpaceId(), $transactionInfo->getTransactionId());
    ...
    {
        try {
            $invoice = $this->container->get('trustpayments_payment.invoice')->getInvoice($transactionInfo->getSpaceId(), $transactionInfo->getTransactionId());
    ...
    {
        try {
            $lineItemVersion = $this->container->get('trustpayments_payment.transaction')->getLineItemVersion($transactionInfo->getSpaceId(), $transactionInfo->getTransactionId());
    ...
    {
        try {
            $refunds = $this->container->get('trustpayments_payment.refund')->getRefunds($transactionInfo->getSpaceId(), $transactionInfo->getTransactionId());
            if (is_array($refunds)) {
                return $refunds;
            }
        } catch (\Exception $e) {
