...
use TrustPaymentsPayment\Components\Provider\PaymentMethod as PaymentMethodProvider;
...
class PaymentMethodConfiguration
{
    ...
    public function synchronize()
    {
        ...
        foreach ($this->modelManager->getRepository(Shop::class)->findAll() as $shop) {
            ...
            if ($spaceId && ! in_array($spaceId, $spaceIds)) {
                $paymentMethodConfigurationService = new \TrustPayments\Sdk\Service\PaymentMethodConfigurationService($this->apiClient);
                $configurations = $paymentMethodConfigurationService->search($spaceId, new \TrustPayments\Sdk\Model\EntityQuery());
    ...
    private function updatePaymentMethods()
    {
        $defaultLanguage = $this->modelManager->getRepository(Shop::class)
            ...
            ->getLocale()
            ->getLocale();
        $pluginName = $this->container->getParameter('trust_payments_payment.plugin_name');
        ...
        foreach ($configurations as $configuration) {
            ...
            $payment = $this->paymentInstaller->createOrUpdate($pluginName, [
                'name' => 'trustpayments_' . $configuration->getId(),
                ...
                'action' => 'TrustPaymentsPaymentPay'
            ]);
     ...
     * @return string
     */
    private function getConfigurationState(\TrustPayments\Sdk\Model\PaymentMethodConfiguration $configuration)
    {
