...
//{block name="backend/trustpayments_payment_transaction/controller/transaction"}
Ext.define('Shopware.apps.TrustPaymentsPaymentTransaction.controller.Transaction', {
    ...
    init:function () {
        ...
        me.control({
            'trustpayments-payment-transaction-details': {
            ...
            'trustpayments-payment-transaction-line-items': {
    ...
    onUpdateLineItems: function(record, grid) {
        ...
        Ext.Ajax.request({
            url: '{url controller="TrustPaymentsPaymentTransaction" action="saveLineItem"}',
            params: params,
            success: function(response) {
                var data = Ext.decode(response.responseText);
                if (data.success) {
                    Shopware.Notification.createGrowlMessage(me.snippets.growlTitle, me.snippets.message.success);
                    me.subApplication.getStore('Shopware.apps.TrustPaymentsPaymentTransaction.store.Transaction').load({
                        params: {
    ...
    onDownloadInvoice: function(record, view) {
        window.open('{url controller="TrustPaymentsPaymentTransaction" action=downloadInvoice}?id=' + record.get('id'), 'download');
    },
    
    onDownloadPackingSlip: function(record, view) {
        window.open('{url controller="TrustPaymentsPaymentTransaction" action=downloadPackingSlip}?id=' + record.get('id'), 'download');
    },
    ...
    onVoidTransaction: function(record, view) {
        ...
        if (record.get('canVoid')) {
            me.process(record, view, '{url controller="TrustPaymentsPaymentTransaction" action="void"}',
    ...
    onCompleteTransaction: function(record, view) {
        ...
        if (record.get('canComplete')) {
            me.process(record, view, '{url controller="TrustPaymentsPaymentTransaction" action="complete"}',
    ...
    onDenyDeliveryIndication: function(record, view) {
        ...
        if (record.get('canReview')) {
            me.process(record, view, '{url controller="TrustPaymentsPaymentTransaction" action="deny"}',
    ...
    onAcceptDeliveryIndication: function(record, view) {
        ...
        if (record.get('canReview')) {
            me.process(record, view, '{url controller="TrustPaymentsPaymentTransaction" action="accept"}',
    ...
    onUpdateTransaction: function(record, view) {
    		...
    		me.process(record, view, '{url controller="TrustPaymentsPaymentTransaction" action="update"}',
