...
if (typeof ShopwareTrustPayments == 'undefined') {
    var ShopwareTrustPayments = {};
}

ShopwareTrustPayments.Checkout = {
    handler : null,
    ...
    attachListeners: function(){
        $('#confirm--form').on('submit.trustpayments_payment', $.proxy(function(event){
            this.onSubmit();
    ...
    createHandler : function(container, configurationId) {
        ...
        if (!this.handler) {
            ...
            this.handler.setHeightChangeCallback(function(height){
            		if (height > 0) {
            			$('#trustpayments_payment_method_form_container').css({
            				position: 'static',
            				left: 'auto'
            			});
                }
    ...
    showErrors: function(errors){
    		var element = $('.trustpayments-payment-validation-failure-message');
